...
from src.explain import generate_explanation, check_explanation_score_consistency
...
def run_benchmark():
    ...
    consistency_df.to_csv("results/consistency_flags.csv", index=False)
    print(f"Consistency flags saved to results/consistency_flags.csv ({len(consistency_df)} flagged)")
